/*******************************************************************************
**
** FileName: APIWrapper.js (renamed to "comm-support.js" for uPerform)
**
*******************************************************************************/
...
********************************************************************************
**
**	Updated by Sean K. Friese at RWD Technologies, LLC. for uPerform
**		- _Debug flag set to debug option in Conf object
**		- Alerts now controlled by _Debug flag
**
*******************************************************************************/

var _Debug = Conf.API_WRAPPER_DEBUG;	// set this to false to turn debugging off
...
function doGetValue(name)
{
   ...
   else
   {
	  ...
	  // Updated 3/10/2010 - skf: Error code 403 shouldn't keep us from getting a value
      if (errCode != _NoError && errCode != "403")
      {
...
function findAPI(win)
{
   if(!Utils.window.isSameDomain(win)){return;} // Added by SKF @ Ancile to avoid cross-domain errors
	...
	while ((win.API_1484_11 == null) && (win.parent != null) && (win.parent != win))
	{
		...
		if(Utils.window.isSameDomain('parent')) // Added by SKF @ Ancile to avoid cross-domain errors
		{
			win = win.parent;
		}
	}
...
function getAPI()
{
	// First we look in the current window...
	var theAPI = findAPI(window);

	// If we can't find it in this window's lineage, look at it's opener's lineage...
